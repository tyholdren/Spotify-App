...
class SubGenres extends Component {
  ...
  getSubGenres() {
    spotifyWebApi.getMyTopArtists({ limit: 50 })
      .then((response) => {
        let genresArray = [];
  ...
  render() {
    return (
      <div>
        <button onClick={this.getSubGenres} className="infoButtons">See My Subgenres</button>
        <div>

          {this.state.show && Object.entries(this.state.subGenres).map((genre, index) => {

            genre[1] = genre[1].filter((e, i) => genre[1].indexOf(e) === i).map(element => {
              return ` ${element}`
            })
            if (genre[1].length === 0) genre[1] = 'none'
            return <SubGenreItem key={index} genre={`${genre[0]}: ${genre[1]}`} />
          })}
        </div>
      </div>
    )
